...
const AnunciosUsuario = () => {
  ...
  const getMyAds = () => {
    axios.get(apiUrl + "user-ads", { headers: getAuthHeaders() })
      .then((res) => {
  ...
  return (
    <section className={styles.adlistSection}>
      ...
      <p className={styles.adlistDescription}>
        Encontre os anúncios que você publicou, nessa aba!
      </p>
      {isLoading ? (
      ...
      {myAds.length == 0 && isLoading == false ? (
        <div style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", border: "solid 1px #8080804d", borderRadius: "10px" }}>
          <div>Nenhum anúncio publicado ainda!</div>
        </div>
      ) : (<></>)}
